<x-filament-panels::page>
    ...
    {{-- Attendance by Level --}}
    @if(!empty($levelChartData) && !empty($levelChartData['labels']))
        <div class="report-grid mb-6">
            <x-filament::section>
                <x-slot name="heading">{{ __('Attendance by Level') }}</x-slot>
                <div
                    wire:ignore
                    x-data="{ chart: null }"
                    x-init="
                        ...
                        loadChartJs().then(() => render($wire.levelChartData));
                        $wire.$watch('levelChartData', (newData) => render(newData));
                    "
                >
            ...
            <x-filament::section>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody>
                            @foreach($levelTableData as $row)
                                <tr class="border-b dark:border-gray-600">
                                    <td class="px-4 py-2">{{ $row['label'] }}</td>
                                    @foreach($attendanceTypes as $type)
                                        <td class="px-4 py-2 text-right">{{ $row['type_' . $type['id']] }}</td>
                                    @endforeach
    ...
    @else
        <x-filament::section class="mb-6">
            <x-slot name="heading">{{ __('Attendance by Level') }}</x-slot>
            <p class="text-sm text-gray-500">{{ __('No attendance data available.') }}</p>
        </x-filament::section>
    @endif

    {{-- Attendance by Rhythm --}}
    @if(!empty($rhythmChartData) && !empty($rhythmChartData['labels']))
        <div class="report-grid">
            <x-filament::section>
                <x-slot name="heading">{{ __('Attendance by Rhythm') }}</x-slot>
                <div
                    wire:ignore
                    x-data="{ chart: null }"
                    x-init="
                        ...
                        loadChartJs().then(() => render($wire.rhythmChartData));
                        $wire.$watch('rhythmChartData', (newData) => render(newData));
                    "
                >
                    <canvas style="max-height: 300px;"></canvas>
                </div>
            ...
            <x-filament::section>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody>
                            @foreach($rhythmTableData as $row)
                                <tr class="border-b dark:border-gray-600">
                                    <td class="px-4 py-2">{{ $row['label'] }}</td>
                                    @foreach($attendanceTypes as $type)
                                        <td class="px-4 py-2 text-right">{{ $row['type_' . $type['id']] }}</td>
